...
function mrs_pdf_splitter_process() {
    ...
    $res = \MRS_CS\handle_upload_field('file', ['pdf'], 50 * 1024 * 1024);
    ...
    try {
        ...
        $pdfCount = $fpdi->setSourceFile($uploaded);
    } catch (\Exception $e) {
    ...
    if (empty($pagesInput)) {
        ...
        for ($p = 1; $p <= $pdfCount; $p++) {
            ...
            $tplId = $pdfOut->setSourceFile($uploaded);
            ...
            $out = \MRS_CS\temp_dir() . pathinfo($uploaded, PATHINFO_FILENAME) . '_page_' . $p . '.pdf';
            $pdfOut->Output($out, 'F');
            $outFiles[] = $out;
        }
    } else {
    ...
    if (count($outFiles) === 1) {
        ...
        $zip = new \ZipArchive();
        $zipPath = \MRS_CS\temp_dir() . 'split_' . time() . '.zip';
